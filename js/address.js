	...
	if ($('#inputbalance').val() != "") {
	 try {
		ethers.utils.getAddress(address); //ethers.js kütüphanesinden adres kontrolü yapar	
			...
			$(".modalcontent").html("Your Wrapped ILCOIN address is valid");
			$("#inputbalance").val("");
		} catch (error) {
		   $('#modalresult').modal('show');
			$(".modal-title").html('<i style="color:white" class="fas fa-exclamation-circle"></i> Invalid Address');
			$(".modalcontent").html("Your Wrapped ILCOIN address is invalid. This validation is made from ethers.js library.");
			$("#inputbalance").val("");
		  return;
        }	
	} else {
